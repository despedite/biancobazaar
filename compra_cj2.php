...
$actualiza = "UPDATE productos SET cantidad=cantidad - 1 WHERE cod_art = 1";
...
$tparc = mysqli_query($dbcon, "SELECT precio_unit FROM productos WHERE cod_art = 1");
$result3 = mysqli_fetch_array($tparc);
$totalparcial = $result3['precio_unit'];
echo $totalparcial;

# Agarra el total definitivo de la tabla de productos.
$totes = mysqli_query($dbcon, "SELECT total FROM productos WHERE cod_art = 1");
